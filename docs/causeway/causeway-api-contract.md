...
`key` uses stable navigation keys (`all`, `hot`, `new`, `politics`, `sports`, `crypto`, `macro`, `tech`, `entertainment`, `other`). The frontend may hide `all` while keeping the API count available for future navigation.
